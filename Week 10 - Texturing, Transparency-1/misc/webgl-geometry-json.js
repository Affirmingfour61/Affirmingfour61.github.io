...
class WebGLGeometryJSON {
    ...
    // -----------------------------------------------------------------------------
    getPosition() {
        // Returns a vector4 of this object's world position contained in its matrix
    ...
    create(jsonFileData, rawImage) {
        ...
        if (rawImage) {
            ...
            // todo #6 - set up trilinear filtering (generate mipmaps)
    ...
    render(camera, projectionMatrix, shaderProgram) {
        ...
        if (this.texture && uniforms.textureUniform) {
            ...
            this.gl.bindTexture(this.gl.TEXTURE_2D, this.texture);
            // Send texture unit to shader (usually 0)
